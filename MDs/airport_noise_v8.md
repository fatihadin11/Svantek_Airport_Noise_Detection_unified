...
Modüler yapı:
  ...
  7. AirportNoiseSystem – Orkestratör (ML modeli kullanır)

DEĞİŞİKLİK (entegrasyon):
  - AirportNoiseSystem artık eğitilmiş SVM modelini kullanıyor
  - extract_features_ml() → dataset_builder_v2.py ile aynı (264 boyut)
  - Kural tabanlı sınıflandırıcı yedek olarak kaldı
"""
...
class AirportNoiseSystem:
    ...
    PRIOR_WEIGHTS = {
        "AIRCRAFT": 0.10,   # ← Ana ayar: düşürürsen AIRCRAFT daha az seçilir
        "SPEECH":   2.0,
        "TRAFFIC":  2.0,
    ...
    def _apply_prior(self, probs: np.ndarray,
        ...
        """
        Ham model olasılıklarına eğitim dengesi düzeltmesi uygula.
        classes belirtilmezse self.ml_le.classes_ kullanılır.
        """
        if classes is None:
            classes = list(self.ml_le.classes_)
        adjusted = probs.copy()
    ...
    def _classify_cnn(self, audio_path: str):
        ...
        # EfficientNet ile aynı 5-değer imzası: (frame_labels, label_times, summary, frame_probs, class_names)
        return frame_labels, label_times, summary, frame_adj_probs, classes
        ...
        with torch.no_grad():
            for chunk in chunks:
                try:
                    ...
                    spec_rgb = to_rgb(spec_rgb).unsqueeze(0)        # (1, 3, 224, 224)
    ...
    def analyze_for_gui(self, audio_path: str, model_pref: str = "auto") -> dict:
        ...
        if model_pref == "efficientnet" and self.eff_model is not None:
            ...
            fl, lt, sm, fp, cn = self._classify_cnn(audio_path)
            used = "CNN"
    ...
    def _infer_efficientnet_chunk(self, chunk: np.ndarray) -> tuple:
        ...
        with torch.no_grad():
            ...
            spec_rgb = to_rgb(spec_rgb).unsqueeze(0)
            logits   = self.eff_model(spec_rgb)
            probs    = torch.softmax(logits, dim=1).squeeze().numpy()
            adj      = self._apply_prior(probs, classes)
    ...
    def _infer_cnn_chunk(self, chunk: np.ndarray) -> tuple:
        ...
        with torch.no_grad():
            y_t   = torch.FloatTensor(chunk).unsqueeze(0)
            spec  = db_tf(mel_tf(y_t)).unsqueeze(0)
            logits = self.cnn_model(spec)
            probs  = torch.softmax(logits, dim=1).squeeze().numpy()
            adj    = self._apply_prior(probs, classes)
            label  = classes[int(np.argmax(adj))]
    ...
    def classify_chunk_live(self, chunk: np.ndarray,
        ...
        model_pref: "auto" | "efficientnet" | "cnn" | "svm"

        Döndürür:
            label   : str  — kazanan sınıf
            probs   : dict[str, float]  — prior-düzeltmeli olasılıklar
